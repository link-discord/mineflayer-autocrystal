...
class AutoCrystal {
    ...
    findPosition(entity) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const entity_position = new vec3_1.Vec3(
            ...
            positions = positions.filter(
                (block) => this.bot.blockAt(block.offset(0, 1, 0)).name === 'air'
            )
            if (this.options.placeMode === 'damage' && positions && positions.length >= 1) {
                try {
                    ...
                    const killPosition = arr.find((pos) => {
                        return (
                            pos.enemyDamage >= entity.health &&
                            pos.selfDamage < this.bot.entity.health
                        )
                    })
    ...
    breakCrystal(crystal) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.bot.emit(
                'debug',
                `[AutoCrystal] Crystal entity: ${
                    crystal === null || crystal === void 0 ? void 0 : crystal.id
                }`
            )
            if (crystal) {
                const damage = this.bot.getExplosionDamages(
            ...
            while (this.run) {
                ...
                if (player && crystal) {
                    ...
                    try {
                        ...
                        if (position) {
                            this.debug(
                                `Position where crystal will be placed: ${position.toString()}.`
                            )
                            const placed = yield this.placeCrystal(position)
                            if (placed) yield this.breakCrystal()
                        }
                    } catch (error) {
                        this.run = false
                        if (this.options.logErrors) this.bot.emit('error', error)
